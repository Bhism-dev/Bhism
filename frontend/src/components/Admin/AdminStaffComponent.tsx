...
const AdminStaffComponent: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        const response = await fetch("https://bhismbackend.hsdev.workers.dev/doctors/doctors", {
          method: "GET",
  ...
  const handleDeleteDoctor = async (doctorId: string) => {
    try {
      const response = await fetch(`https://bhismbackend.hsdev.workers.dev/doctors/${doctorId}`, {
        method: "DELETE",
      });
  ...
  const handleToggleAvailability = async (doctorId: string, availability: boolean) => {
    try {
      const response = await fetch(`https://bhismbackend.hsdev.workers.dev/doctors/${doctorId}/availability`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleEditDoctor = async () => {
    if (selectedDoctor) {
      try {
        const response = await fetch(`https://bhismbackend.hsdev.workers.dev/doctors/${selectedDoctor.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
