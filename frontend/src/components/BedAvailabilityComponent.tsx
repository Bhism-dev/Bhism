...
const BedAvailabilityComponent: React.FC = () => {
  const beds: {
    Hospital: {
      [key: string]: {
        ...
        bed: { [key: string]: { [key: number]: boolean } };
      };
    };
  } = {
    Hospital: {
  ...
  const floorsForWard =
    selectedHospital && selectedWard
      ? beds.Hospital[selectedHospital].floor[selectedWard]
      : [];
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>Bed Availability</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent className="ion-padding">
        ...
        {selectedHospital && (
          <IonList>
            ...
            >
              {beds.Hospital[
                selectedHospital as keyof typeof beds.Hospital
              ].ward.map((ward) => (
            ...
            <IonCard>
              <IonCardContent>
                <IonText className="font-bold text-lg">
                  {selectedWard} Ward - Bed Availability on Floor{" "}
                  {selectedFloor}
                </IonText>
                <IonGrid>
                  <IonRow>
                    {Object.entries(
                      beds.Hospital[selectedHospital].bed[selectedWard]
                    )
                      .filter(
                        ([room]) =>
                          Math.floor(Number(room) / 100) === selectedFloor
                      )
                      .map(([room, available]) => (
